...
export default function Table() {
    ...
    const handleDelete = (idProduct) => {
        axios.delete(`http://localhost:8000/api/product/delete/${idProduct}`).then(res => {
                                    ...
                                    {products.length > 0 && products.map((product, index) => {
                                        return (
                                            <tr>
                                                ...
                                                <td class="text-center px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{product.price &&(product.price).toLocaleString()}</td>
                                                <td class="text-center px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                    <div className="flex justify-center">
                                                        ...
                                                        <Modal isOpen={isModalOpen} onClose={handleCloseModal}  select = {indexSelect} submit={handleSubmitForm}/>
